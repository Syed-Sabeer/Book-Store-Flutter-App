...
class ShippedViewPage extends StatelessWidget {
  ...
  const ShippedViewPage({super.key, required this.orderId, required this.order});
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoRow('Postal Code:', (order['postalCode'] ?? 'N/A').toString()),
                ...
                ElevatedButton(
                  onPressed: () async {
                    try {
                      ...
                      // Add the current order to the 'deleted_orders' collection
                      await firestore.collection('deleted_orders').add(order);

                      // Delete the order from the 'orders' collection using the passed orderId
                      await firestore.collection('shipped_orders').doc(orderId).delete();
                ...
                ElevatedButton(
                  onPressed: () async {
                    try {
                      ...
                      // Add the current order to the 'completed_orders' collection
                      await firestore.collection('completed_orders').add(order);
                      ...
                      Navigator.pop(context); // Go back after marking as completed
                    } catch (e) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Error marking order as completed: $e')),
                      );
                    }
                  },
                  style: ElevatedButton.styleFrom(
